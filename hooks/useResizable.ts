...
type ResizeDirection = 'br' | 'tr' | 'tl';

interface UseResizableOptions {
    onStart?: () => void;
    onStop: (updates: { size: { width: number; height: number }; position: { x: number; y: number } }) => void;
...
export const useResizable = (
    ...
    const handleTouchMove = useCallback((e: TouchEvent) => { if (e.touches.length > 0) handleMove(e.touches[0].clientX, e.touches[0].clientY); }, [handleMove]);
    ...
    const handleStart = useCallback((clientX: number, clientY: number, direction: ResizeDirection) => {
        onStart?.();
    ...
    const handleMouseDown = useCallback((e: React.MouseEvent<HTMLDivElement>, direction: ResizeDirection) => {
        e.stopPropagation(); e.preventDefault();
        handleStart(e.clientX, e.clientY, direction);
    }, [handleStart]);

    const handleTouchStart = useCallback((e: React.TouchEvent<HTMLDivElement>, direction: ResizeDirection) => {
        e.stopPropagation(); e.preventDefault();
