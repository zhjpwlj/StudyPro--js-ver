...
import { WidgetInstance, WidgetType, Task, Habit, JournalEntry, CalendarEvent } from '../types';
...
import { WIDGET_HEADER_HEIGHT, DOCK_AREA_HEIGHT } from '../constants';

interface DesktopProps {
    onLogout: () => void;
}

const componentMap: { [key in WidgetType]: React.FC<any> } = {
    tasks: TaskList,
    chatbot: Chatbot,
    habits: HabitTracker,
...
const Desktop: React.FC<DesktopProps> = ({ onLogout }) => {
    ...
    const [widgets, setWidgets] = usePersistentState<WidgetInstance[]>('studypro-widgets', defaultWidgets);
    const [tasks, setTasks] = usePersistentState<Task[]>('studypro-tasks', initialTasks);
    const [habits, setHabits] = usePersistentState<Habit[]>('studypro-habits', initialHabits);
    const [journalEntries, setJournalEntries] = usePersistentState<JournalEntry[]>('studypro-journal', []);
    const [calendarEvents, setCalendarEvents] = usePersistentState<CalendarEvent[]>('studypro-calendar-events', []);
    ...
    const openWidget = useCallback((widgetType: WidgetType) => {
    ...
    const getWidgetProps = (widget: WidgetInstance) => {
        switch (widget.type) {
