...
import { SunIcon, MoonIcon, MapPinIcon, MicrophoneIcon, SnowflakeIcon } from './icons/Icons';
...
const WeatherWidget: React.FC = () => {
    ...
    const fetchWeather = async (lat: number, lon: number, name: string) => {
        try {
            ...
            const formattedData = {
                ...
                forecast: data.daily.time.slice(1, 6).map((day: string, index: number) => ({
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            if (isEditingLocation && editLocationRef.current && !editLocationRef.current.contains(event.target as Node)) {
