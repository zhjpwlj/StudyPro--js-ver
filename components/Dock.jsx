...
const Dock = ({ onWidgetSelect, openWidgets }) => {
    ...
    const handleItemClick = (item) => {
        if (item.children) {
            ...
            onWidgetSelect(item.key);
        }
    };
    
    const handleFolderItemClick = (widgetType) => {
    ...
    const isFolderActive = (item) => {
        ...
        return item.children.some(child => openWidgets.includes(child.key));
    };
            ...
            <div className="dock-container flex items-end h-20 p-2 space-x-2 rounded-2xl" ref={dockRef}>
                {NAV_ITEMS.map((item) => (
                    ...
                    >
                            ...
                            <div className="absolute bottom-full mb-4 w-52 p-3 rounded-2xl dock-container grid grid-cols-3 gap-y-3 gap-x-2">
                                {item.children.map(child => (
                                     <div key={child.key} className="flex flex-col items-center text-center relative">
                                        <button
                                            onClick={() => handleFolderItemClick(child.key)}
                                            title={t(child.key)}
                                        ...
                                        {openWidgets.includes(child.key) && (
                        ...
                        {(openWidgets.includes(item.key) || isFolderActive(item)) && (
